...
function createAdmin($request_values){
	...
	if (empty($role)) { array_push($errors, "Potreban je admin users");}
	if (empty($password)) { array_push($errors, "Unesi password"); }
	if ($password != $passwordConfirmation) { array_push($errors, "Passwords se razlikuju"); }
	...
	if (count($errors) == 0) {
		...
		$_SESSION['message'] = "Admin user kreiran";
...
function createTopic($request_values){
	...
	if (count($errors) == 0) {
		...
		$_SESSION['message'] = "Vesti kreirane.";
...
function updateTopic($request_values) {
	...
	$topic_slug = makeSlug($topic_name);
	if (empty($topic_name)) { 
		array_push($errors, "Uneti naziv Vesti !!!"); 
	}
	...
	if (count($errors) == 0) {
		...
		$_SESSION['message'] = "Vesti uredjene";
		header('location: topics.php');
		exit(0);
	}
}
...
function deleteTopic($topic_id) {
	...
	if (mysqli_query($conn, $sql)) {
		$_SESSION['message'] = "Vest obrisana";
		header("location: topics.php");
		exit(0);
	}
}
...
function updateAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$_SESSION['message'] = "Admin user uredjen";
		header('location: users.php');
		exit(0);
	}
}
